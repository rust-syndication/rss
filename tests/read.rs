...
fn read_category()
{
    ...
    assert_eq!(channel.categories[1]
                   .domain()
                   .as_ref()
                   .map(|s| s.as_str()),
               Some("http://example.com/"));
    ...
    assert_eq!(channel.items[0].categories()[1]
                   .domain()
                   .as_ref()
                   .map(|s| s.as_str()),
               Some("http://example.com/"));
