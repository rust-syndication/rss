    ...
    while let Some(e) = reader.next() {
        match e {
            ...
            Ok(Event::CData(element)) => {
                ...
                content = Some(try_reader!(str::from_utf8(text),
                                           reader)
                                   .to_string());
            },
            Ok(Event::Text(element)) => {
                let text = try_reader!(element.unescaped_content(),
                                       reader);
                content = Some(try_reader!(String::from_utf8(text.into_owned()),
                                           reader));
            },
            Err(err) => return (Err(err.into()), reader),
            _ => {},
    ...
    -> (Result<(), Error>, XmlReader<R>)
{
    ...
    let ext = parse_extension_element(reader,
                                      element);
    reader = ext.1;
    let ext = try_reader!(ext.0,
                          reader);

    if !extensions.contains_key(ns) {
        extensions.insert(ns.to_string(),
                          HashMap::new());
    }
    ...
    for attr in element.attributes().with_checks(false).unescaped() {
        if let Ok(attr) = attr {
            let key = try_reader!(str::from_utf8(attr.0),
                                  reader);
            let value = try_reader!(str::from_utf8(&attr.1),
                                    reader);
    ...
    while let Some(e) = reader.next() {
        match e {
            Ok(Event::Start(element)) => {
                let child = parse_extension_element(reader,
                                                    &element);
                reader = child.1;
                let ext = try_reader!(child.0,
                                      reader);
                ...
                let name = {
                    let split = element.name()
                                       ...
                                       .collect::<Vec<_>>();
                    if split.len() == 2 {
                        try_reader!(str::from_utf8(unsafe { split.get_unchecked(1) }),
                                    reader)
                    } else {
                        try_reader!(str::from_utf8(unsafe { split.get_unchecked(0) }),
                                    reader)
                    }
                };
