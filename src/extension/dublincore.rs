...
impl DublinCoreExtension
{
    ...
    pub fn from_map(mut map: HashMap<String, Vec<Extension>>) -> Self
    {
        ...
        ext.contributor = remove_extension_values(&mut map,
                                                  "contributor")
            .unwrap_or_default();
        ext.coverage = remove_extension_values(&mut map,
                                               "coverage")
            .unwrap_or_default();
        ext.creator = remove_extension_values(&mut map,
                                              "creator")
            .unwrap_or_default();
        ext.date = remove_extension_values(&mut map,
                                           "date")
            .unwrap_or_default();
        ext.description = remove_extension_values(&mut map,
                                                  "description")
            .unwrap_or_default();
        ext.format = remove_extension_values(&mut map,
                                             "format")
            .unwrap_or_default();
        ext.identifier = remove_extension_values(&mut map,
                                                 "identifier")
            .unwrap_or_default();
        ext.language = remove_extension_values(&mut map,
                                               "language")
            .unwrap_or_default();
        ext.publisher = remove_extension_values(&mut map,
                                                "publisher")
            .unwrap_or_default();
        ext.relation = remove_extension_values(&mut map,
                                               "relation")
            .unwrap_or_default();
        ext.rights = remove_extension_values(&mut map,
                                             "rights")
            .unwrap_or_default();
        ext.source = remove_extension_values(&mut map,
                                             "source")
            .unwrap_or_default();
        ext.subject = remove_extension_values(&mut map,
                                              "subject")
            .unwrap_or_default();
        ext.title = remove_extension_values(&mut map,
                                            "title")
            .unwrap_or_default();
        ext.resource_type = remove_extension_values(&mut map,
                                                    "type")
            .unwrap_or_default();
        ext
    }
